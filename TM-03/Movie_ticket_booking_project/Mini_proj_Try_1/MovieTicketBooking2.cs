...
namespace MovieTicketBooking2
{
    ...
    {
        public int BookTicket(int ShowID, string CustomerName, int Numberofseats, string Seattype, string Email)
        {
            bool flag = false;
            Booking obj = new Booking(ShowID, CustomerName, Numberofseats, Seattype, Email);
            try
            {
                if (obj == null)
                {
                ...
                else if (ShowID == null || CustomerName == "" || Numberofseats == null || Seattype == "" || Email == "")
                {
        ...
        public void PrintTicket(int BookingID)
        {
            ...
            else
            {
                ...
                {
                    if (BookingID == val.BookingID)
                    {
                        Console.WriteLine("Booking ID:{0}\tBooking Date:{1}\tCustomer Name:{2}", val.BookingID, val.BookingDate, val.CustomerName);
                        Console.WriteLine("Seats:{0}\tSeat type:{1}\tSeat Numbers:{2}", val.NumberOfSeats, val.SeatType, val.SeatNumbers);
                        flag = true;
                        sid = val.ShowID;
                        break;
                    }
                ...
                {
                    Console.WriteLine("BookingID not found");
                }
                ...
                if (flag)
                {
                    foreach (var val in MovieTicketing.Movies)
                    {
                        if (mid == val.MovieID)
                        {
                            Console.WriteLine("Movie Name:{0}\tShow ID:{0}\tTheatre ID/Screen ID:{1}", val.MovieName, tid, scid);
                        }
                    }
                }
                foreach (var val in MovieTicketing.Bookings)
                {
                    if (BookingID == val.BookingID)
                    {
                        Console.WriteLine("Amount Paid:{0}\t\t\tBooking Status:{1}", val.Amount, val.BookingStatus);
                    }
                }
            }
        }
    ...
    public class Administrator : TicketBooking,IAdmin
    {
